...
hive all が正常に完了しても、adminユーザが認証エラーでログインできない場合があります。
IDManager V3 の管理画面にバックドアでログインし、プロビジョニングタスクが正常に完了しているかどうか確認して下さい。
バックドアログインの手順は以下の通りです。
1. ポートフォワーディングでIDManager V3の管理画面にアクセスできるようにします。
   ```
   hive ssh -L 10443:0.0.0.0:10443
   ```
2. ブラウザで https://localhost:10443 にアクセスします。
3. ログイン画面で以下の情報を入力してログインします。
    | 項目 | 値 |
    | --- | --- |
    | ユーザー名 | IDM_ADMIN |
    | パスワード | netso@rer |
4. ベース設定 > プロビジョニング設定 > タスク閲覧 から失敗しているプロビジョニングタスクがないか確認します。
5. 失敗しているプロビジョニングタスクがある場合は、タスクの再試行を行います。